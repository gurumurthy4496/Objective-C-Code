...
- (void)setInitializingMemoryForServicesBeingPerformedTodayTableView {
    ...
    [tableView setFrame:CGRectMake(0, 30, self.frame.size.width, 260)];
...
- (void)setTextForSubTotalOfServicesListedCost :(float)aSubTotalOfServicesListedCost ShopChargesAndTaxesCost :(float)aShopChargesAndTaxesCost TotalCost :(float)aTotalCost {
    self.mSubTotalOfServicesListedLabel_.text = [NSString stringWithFormat:@"%.2f",aSubTotalOfServicesListedCost];
    self.mShopChargesAndTaxesLabel_.text = [NSString stringWithFormat:@"%.2f",aShopChargesAndTaxesCost];
    self.mTotalLabel_.text = [NSString stringWithFormat:@"%.2f",aTotalCost];
}
