...
-(void)loadServices{
    ...
    if(  mAppDelegate_.mServiceDataGetter_.mAllServicesArray_==nil)
    {
        ...
        mAppDelegate_.mModelForServiceRequestWebEngine_->isLoadingRequired=FALSE;
...
-(NSMutableArray*)getServiceLines{
    ...
    for(int i=0;i<[mSGIDsArray_ count];i++)
    {
        for (int j=0; j<[lServicesArray count]; j++) {
            if([[NSString stringWithFormat:@"%@",[[lServicesArray objectAtIndex:j] objectForKey:@"SGID"]] isEqualToString:[NSString stringWithFormat:@"%@",[mSGIDsArray_ objectAtIndex:i]]])
