...
-(void)loadInspectionItems{
    ...
    NSLog(@"%@",mAppDelegate_.mOpenROInspectionDataGetter_.mInspectionFormDict_ );
...
-(void)loadServices{
    ...
    if(  mAppDelegate_.mServiceDataGetter_.mAllServicesArray_==nil)
    {
        ...
        mAppDelegate_.mModelforOpenROSupportEngine_->isLoadingRequired=FALSE;
...
-(void)setCompleteInspectionButton{
    if (([[[[SharedUtilities sharedUtilities] appDelegateInstance] mModelForSplashScreen_].mEmployeeType_ isEqualToString:@"Technician"])&&([[[[SharedUtilities sharedUtilities] appDelegateInstance]mModelForVehicleHistoryTableView_]mCurrentMode_] ==3)) {
        isMandatory=TRUE;
    }
    else
        isMandatory=FALSE;
