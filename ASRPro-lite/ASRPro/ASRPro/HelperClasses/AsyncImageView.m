...
- (void)setImage:(UIImage *)image forURL:(NSURL *)URL
{
  ...
  /*  CGSize newSize;
    if([image size].height>(150*2)){
        newSize.height=(150*2);
        newSize.width=(150*2)*[image size].width/([image size].height);
    }
    else{
    ...
    [[DataBaseUtilsClass instance] saveDataInToFile:pictureData :@"abcd" :@"dk"];*/
...
- (void)imageFailed:(NSNotification *)notification
{
    ...
    for (int i = [connections count] - 1; i >= 0; i--)
    {
        ...
        if ([connection.URL isEqual:URL])
        {
            ...
            //TO ADD DUMMY IMAGE IN FAILURE CASE
//            if ([[connection.target parentName] isEqualToString:PATIENT_IMAGE_TYPE]) {
//                [connection.target setImage:[UIImage imageNamed:@"my-profile.png"]];
//                
//            }else if ([[connection.target parentName] isEqualToString:TRACKER_IMAGE_TYPE]) {
//                [connection.target setImage:[UIImage imageNamed:@"Tracker.png"]];
//                
//            }else if ([[connection.target parentName] isEqualToString:HOSPITAL_IMAGE_TYPE]) {
//                [connection.target setImage:[UIImage imageNamed:@"hsptl-default.png"]];
//            }
//                 else if ([[connection.target parentName] isEqualToString:DOCTOR_IMAGE_TYPE]) {
//                     [connection.target setImage:[UIImage imageNamed:@"my-profile.png"]];
               //  }
            //remove from queue
            [connections removeObjectAtIndex:i];
...
- (void)setImage:(UIImage *)image
{
    ...
    if (image==nil) {
        if([[AsyncImageCache sharedCache] deaultimag]!=nil){
       super.image = [[AsyncImageCache sharedCache] deaultimag];
        }
