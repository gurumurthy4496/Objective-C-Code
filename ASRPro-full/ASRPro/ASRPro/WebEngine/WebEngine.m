...
//    [theRequest setValue:@"Basic Q2hlY2tJbkFwcDowM2E1YkU4MDY4ZEM5YyA4ZGUyNDc=" forHTTPHeaderField:@"Authorization"];
    [theRequest setValue:@"Basic Q2hlY2tJbkFwcDowM2E1YkU4MDY4ZEM5YyA4ZGUyNDc=" forHTTPHeaderField:@"Authorization"];
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    NSString *response1 = [[NSString alloc] initWithBytes:[webData mutableBytes]
                                                   length:[webData length]
                                                 encoding:NSUTF8StringEncoding];
    DLog(@"Response ->%@",response1);
}
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
	if([webData length]==0 && mResponseCode_ !=ASRProOKStatusCode&&mResponseCode_!=ASRProCreatedStatusCode) {
        ...
        if(mResponseCode_ ==ASRProOKStatusCode||mResponseCode_==ASRProCreatedStatusCode||mResponseCode_==ASRProHTTPInternalErrorCode) {
            ...
            }else if ([mAppDelegate_.mWebEngine_.mWebMethodName_ isEqualToString:kEDIT_CUSTOMER_DETAILS] || [mAppDelegate_.mWebEngine_.mWebMethodName_ isEqualToString:kEDIT_VEHICLE_DETAILS]) {
                if ([mAppDelegate_.mWebEngine_.mWebMethodName_ isEqualToString:kEDIT_CUSTOMER_DETAILS]) {
                    [[SharedUtilities sharedUtilities] showAlertWithTitle:NSLocalizedString(@"lERROR", nil) message:[NSString stringWithFormat:@"Customer : Error response from server : %@",json_string]];
                }else
                    [[SharedUtilities sharedUtilities] showAlertWithTitle:NSLocalizedString(@"lERROR", nil) message:[NSString stringWithFormat:@"Vehicle : Error response from server : %@",json_string]];
            }
		}
	}
...
- (void)showErrorResponseForLogInView {
    ...
    if (mResponseCode_ == ASRProForbiddenStatusCode) {
        [mAppDelegate_.mSplashScreenViewController_.mErrorMessageLabel_ setText:NSLocalizedString(@"lAccount_Access", nil)];
