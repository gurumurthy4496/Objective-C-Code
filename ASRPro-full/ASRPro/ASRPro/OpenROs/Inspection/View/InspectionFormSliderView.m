...
@property (nonatomic,retain)CustomSlider *mSlider_;
...
-(void)displaySliderTopValues {
    ...
    if([itemsarray count]>0){
        CGRect frame = CGRectMake(290, 0,310,15);
        ...
        [lMinButton_ setFrame:CGRectMake(255, 5, 25, 25)];
        ...
        [self addSubview:mLabelAboveSlider_];
            frame = CGRectMake(290, 20, 310, 10.0);
        
        mSlider_ = [[CustomSlider alloc] initWithFrame:frame];
        ...
        [lPlusButton_ setFrame:CGRectMake(610, 5, 25, 25)];
...
-(void)addRadioButtons{
    for (int i=0;i<[mSubitemsArray_ count];i++){
        InspectionButton* lInspectionCheckButton=[[InspectionButton alloc]initWithFrame:CGRectMake((mFormtype==LANEINSPECTION)?640: 610, 5, 50, 25)];
...
- (void)sliderTapped:(UIGestureRecognizer *)g {
    CustomSlider* s = (CustomSlider*)g.view;
    if (s.highlighted)
