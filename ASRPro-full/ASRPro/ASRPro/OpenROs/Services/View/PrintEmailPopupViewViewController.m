...
-(void)initTheViews{
  ...
  //  [mInspectionFormTextField_ setText:@"test@test.com"];
    ...
    /**
     * Below condition is for ->o   Remove pre RO estimate from lite version (services are not used with lite version)
     */
    if (![[[[SharedUtilities sharedUtilities] appDelegateInstance] isCustomHeaderViewFullOrLight] isEqualToString:@"FULL"]) {
        [self.preRoEstimateLabel setHidden:YES];
        [self.mPreROEstimate setHidden:YES];
    }else {
        [self.preRoEstimateLabel setHidden:NO];
        [self.mPreROEstimate setHidden:NO];
    }
...
- (IBAction)sendMailTypeAction:(id)sender {
    ...
    if(mEstimatebtn_.isSelected||mMPIbtn_.isSelected||mBookletbtn_.isSelected || self.mCustomerPlan || self.mPreROEstimate)
        [mPrintBtn setEnabled:TRUE];
    else
        [mPrintBtn setEnabled:FALSE];

    if((mEstimatebtn_.isSelected||mMPIbtn_.isSelected||mBookletbtn_.isSelected || self.mCustomerPlan || self.mPreROEstimate)&&(mClientMailBtn_.isSelected||mCustomMailBtn.isSelected))
...
- (IBAction)FormTypeAction:(id)sender {
    //Added Customer Plan and Pre RO Estimate because of below 08/23/14
    //Add customer plan and Repair Order (same as pre RO) radio button to print in the Print modal window of the open RO section
    ...
    if(self.mCustomerPlan.tag != mFormtype.tag)
        [self.mCustomerPlan setSelected:FALSE];
    if(self.mPreROEstimate.tag != mFormtype.tag)
        [self.mPreROEstimate setSelected:FALSE];
...
- (IBAction)PrintAction:(id)sender {
    ...
    else if(self.mCustomerPlan.isSelected){
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mModelforOpenROSupportEngine_ RequestForLoadingBooklet:[[SharedUtilities sharedUtilities]appDelegateInstance].mModelForVehicleHistoryTableView_.mOpenROString_ DocumentType:@"Customer Plan" LineType:@"All" Language:@"EN" EmailType:@"false"];
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_ setMPdfTitle:@"Customer Plan"];
        
    }
    else if(self.mPreROEstimate.isSelected){
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mModelforOpenROSupportEngine_ RequestForLoadingBooklet:[[SharedUtilities sharedUtilities]appDelegateInstance].mModelForVehicleHistoryTableView_.mOpenROString_ DocumentType:@"Pre RO Estimate" LineType:@"All" Language:@"EN" EmailType:@"false"];
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_ setMPdfTitle:@"Pre RO Estimate"];
...
-(void)requestForEmail{
    if(mBookletbtn_.isSelected){
        ...
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mModelforOpenROSupportEngine_ RequestForLoadingBooklet:[[SharedUtilities sharedUtilities]appDelegateInstance].mModelForVehicleHistoryTableView_.mOpenROString_ DocumentType:@"Customer Plan" LineType:@"All" Language:@"EN" EmailType:@"true"];
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_ setMPdfTitle:@"Customer Plan"];
        
    }
    else if(self.mPreROEstimate.isSelected){
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mModelforOpenROSupportEngine_ RequestForLoadingBooklet:[[SharedUtilities sharedUtilities]appDelegateInstance].mModelForVehicleHistoryTableView_.mOpenROString_ DocumentType:@"Pre RO Estimate" LineType:@"All" Language:@"EN" EmailType:@"true"];
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_ setMPdfTitle:@"Pre RO Estimate"];
...
-(void)dismisstheView{
    [self dismissViewControllerAnimated:YES completion:^ {
        ...
        [[[SharedUtilities sharedUtilities] appDelegateInstance].mSearchViewController_ pushToPDFView];
    }];
