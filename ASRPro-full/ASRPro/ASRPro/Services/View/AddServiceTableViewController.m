...
@property(nonatomic,retain) ServicePaytypeDropdown* mServicePaytypeDropdown;
@end
...
@synthesize mServicePaytypeDropdown;
...
-(void)initTheViews{
   [ self setMServiceDetailsView:[self getTextViewWithText:mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mDetails_  height:KSERVICEPOPUP_DETAILS_HEIGHT tag:KSERVICEPOPUP_DETAILS]];
    ...
    self.mServicePaytypeDropdown=[[ServicePaytypeDropdown alloc] initWithFrame:CGRectMake(0, 0, 600, KSERVICEPOPUP_PAYTYPE_HEIGHT)];
    [self.mServicePaytypeDropdown setValue:mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mPayTypeCode_];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section
{
    ...
    if(((section==3)||(section==selectedsection)||(section==12))&&(section!=0))
    return 1;
    else
        return 0;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self resetTableViews:cell cellForRowAtIndexPath:indexPath];
        switch(indexPath.section){
            ...
            case KSERVICEPOPUP_PAYTYPE:{
                [cell.contentView addSubview:mServicePaytypeDropdown];
                break;
            }
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
    switch(indexPath.section){
        ...
        case KSERVICEPOPUP_PAYTYPE:{
            return KSERVICEPOPUP_PAYTYPE_HEIGHT;
            break;
        }
        case KSERVICEPOPUP_HOURS:{
...
{
    if(((section!=selectedsection)||(section==0)||(section==KSERVICEPOPUP_HOURS)||(section==KSERVICEPOPUP_PAYTYPE))&&(section!=3)&&(section!=12))
        return 50.0;
    else
        return 0.0;
}
    ...
    switch(section){
        ...
        case KSERVICEPOPUP_NOTES:{
            ...
            if([mAppDelegate_.mModelForSplashScreen_.mEmployeeType_ isEqualToString:@"Advisor"])
                lNotes=mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mAdvisorNotes_;
            if([mAppDelegate_.mModelForSplashScreen_.mEmployeeType_ isEqualToString:@"Technician"])
                lNotes=mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mTechnicianNotes_;
            if([mAppDelegate_.mModelForSplashScreen_.mEmployeeType_ isEqualToString:@"Manager"])
                lNotes=mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mManagerNotes_;

            return [self getViewWithLabel:lNotes btnTag:section defaultText:[[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mModelForSelectedService_] mServiceHeadingArray_] objectAtIndex:section]];
            break;
        }
        case KSERVICEPOPUP_PAYTYPE:{
            return [self getViewWithLabel:mAppDelegate_.mServiceDataGetter_.mModelForSelectedService_.mPayTypeCode_ btnTag:section defaultText:[[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mModelForSelectedService_] mServiceHeadingArray_] objectAtIndex:section]];
            break;
        }
        case KSERVICEPOPUP_HOURS:{
...
-(UIView*)getViewWithLabel:(NSString*)aText
    ...
    if([aText isEqualToString: @""]){
        UILabel* lTempLabel=[[UILabel alloc] initWithFrame:CGRectMake(60, 0, self.tableView.frame.size.width-60, 50)];
        ...
        UILabel* lTempLabel=[[UILabel alloc] initWithFrame:CGRectMake(60, 0, self.tableView.frame.size.width-60, 30)];
        [[GenericFiles genericFiles] createUILabelWithInstance:lTempLabel labelTitleFont:[UIFont regularFontOfSize:14 fontKey:kFontBoldKey] labelTitle:aDefaultText labelTextColor:[UIColor grayColor] labelTextAlignment:NSTextAlignmentLeft labelBackgroundColor:[UIColor clearColor]];
        [lview addSubview:lTempLabel];
        UILabel* lTextLabel=[[UILabel alloc] initWithFrame:CGRectMake(70, 20, self.tableView.frame.size.width-70, 30)];
    ...
    UIView* lDividerView=[[UIView alloc] initWithFrame:CGRectMake(0, 49, self.tableView.frame.size.width, 2)] ;
    [lDividerView setBackgroundColor:[UIColor colorWithRed:186/255.0 green:186/255.0 blue:186/255.0 alpha:1.0]];
    [lview addSubview:lDividerView];
...
-(UIView*)getViewWithRadioButton:(NSString*)aState{
    ...
    [self getButtonwithTag:lview isSelected:[aState isEqualToString:@"Yellow"]?TRUE:FALSE pos:5 btnTag:0];
    [self getButtonwithTag:lview isSelected:[aState isEqualToString:@"Red"]?TRUE:FALSE pos:50 btnTag:1];
...
{
    UIButton *lTempButton = [[UIButton alloc] initWithFrame:CGRectMake(10, yPos,22, 22)];
    lTempButton.tag=aBtnTag;
    ...
    UILabel* lTempLabel= [[UILabel alloc] initWithFrame:CGRectMake(80, yPos+5,250 , 20)];
...
-(void)isHeaderClicked:(UIButton*)sender{
  ...
  else  if(sender.tag==KSERVICEPOPUP_HOURS)
    {
        selectedsection=(selectedsection==0)?KSERVICEPOPUP_HOURS:0;
    }
   else if(sender.tag==KSERVICEPOPUP_PAYTYPE)
    {
        selectedsection=(selectedsection==0)?KSERVICEPOPUP_PAYTYPE:0;
    }
...
-(void)resignTextField{
    switch(selectedsection){
        case KSERVICEPOPUP_DETAILS:{
            [self.mServiceDetailsView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_NOTES:{
            [self.mServiceNotesView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_LABOR:{
            [self.mServiceLaborView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_PARTS:{
            [self.mServicePartsView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_PRICE:{
            [self.mServicePriceView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_COMPLAINT:{
            [self.mServiceComplaintView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_CAUSE:{
            [self.mServiceCauseView resignFirstResponder];
            break;
        }
        case KSERVICEPOPUP_CORRECTION:{
            [self.mServiceCorrectionView resignFirstResponder];
            break;
        }
