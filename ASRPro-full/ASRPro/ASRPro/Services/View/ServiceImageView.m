...
-(void)addImagesArrayView:(NSMutableArray*)anImageArray{
   ...
   // NSMutableArray*lTempArray=[[NSMutableArray alloc] init];
    if([anImageArray count]>0){
        isImagePresent=TRUE;
        [self setMNewImagesArray_:anImageArray];
    UIButton*mAddPhotoButton=[[UIButton alloc] initWithFrame:CGRectMake(330, 10+23, 65, 65)];
       ...
       // mAddPhotoButton.contentHorizontalAlignment = UIControlContentHorizontalAlignmentCenter;
        ...
        AsyncImageView* lTempImage=[[AsyncImageView alloc] initWithFrame:CGRectMake(90, 10,192, 120)];
        NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@",[[anImageArray objectAtIndex:0] objectForKey:@"ImageUrl"]]];
        //load the image
...
- (void)setImage:(UIImage*)pickedImage {
    ...
    [self addImages:pickedImage imageName:[NSString stringWithFormat:@"%@-%d",kIMAGEPATH,0]];
...
- (void)addImages:(UIImage *)image imageName:(NSString *)imageName{
    ...
    [self.subviews enumerateObjectsUsingBlock:^(id object, NSUInteger index, BOOL *stop) {
        if ([object isKindOfClass:[AsyncImageView class]]) {
            if(((AsyncImageView*)object).tag==0)
...
-(void)deleteObject:(int)tag{
 ...
 //   if(currentTag<100)
    if(isImagePresent){
        ...
        [[FileUtils fileUtils] deleteFileFromFolder:kIMAGEPATH :[NSString stringWithFormat:@"%@-%d",kIMAGEPATH,0]];
...
- (void)sendTheimagesToServer {
   ...
   // for(int i=0;i<(AddedCount-100);i++){
        NSData *pictureData = [[FileUtils fileUtils] loadDataFromFile:kIMAGEPATH Path:[NSString stringWithFormat:@"%@-%d",kIMAGEPATH,0]];
        if(pictureData!=nil){
...
//    }];
   ...
   // for(int i=0;i<[mDeleteImagesArray_ count];i++){
    if(isImageDeleted)
        [lTempArray addObject:[mNewImagesArray_ objectAtIndex:0]];
   // }
