...
- (void)searchBar:(UISearchBar *)theSearchBar textDidChange:(NSString *)searchText {
    ...
    if (self.tag == 1) {
        if ([[[SharedUtilities sharedUtilities] appDelegateInstance] mSearchViewController_].mSearchCategory_ == SearchOpenRO) {
            ...
            [self searchTableView];
            [[[[SharedUtilities sharedUtilities] appDelegateInstance] mMasterViewController_] resetAllData];
         [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_.mSelectedServicesArray_ removeAllObjects];
          [[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_ filterLines:[[SharedUtilities sharedUtilities] appDelegateInstance].mOpenRoServiceDataGetter_.mSelectedServicesArray_];
...
- (void) searchTableView {
    ...
    if (self.tag == 1) {
        ...
        for (int i = 0; i < [[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mAllServicesArray_] count]; i++)
        {
            //OpCode is added beacuse of below.08/23/14
            //Search services by Op code: In services screen in the search box allow user to be able to search by op code.

            NSString *searchString = ([[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mAllServicesArray_] objectAtIndex:i] objectForKey:@"ServiceName"]) != nil ?[[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mAllServicesArray_] objectAtIndex:i] objectForKey:@"ServiceName"] :@"";
            
            NSString *OpCode = ([[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mAllServicesArray_] objectAtIndex:i] objectForKey:@"OpCode"]) != nil ?[[[[[[SharedUtilities sharedUtilities] appDelegateInstance] mServiceDataGetter_] mAllServicesArray_] objectAtIndex:i] objectForKey:@"OpCode"] :@"";
            
            searchString = [searchString stringByAppendingString:OpCode];
