    ...
    if(mAppDelegate_.mPartLabourDataGetter_->isSucess){
        ...
        for(int i=0;i<[mAppDelegate_.mPartLabourDataGetter_.mPartsLookupDict_ count];i++){
            ...
            [mPartsScrollView addSubview:lPartsLabourLookupSubview_];
                     height=(i==0)?lPartsLabourLookupSubview_->rowheight:40;
            ypos+=height;
        }
...
- (IBAction)SaveButtonAction:(id)sender {
    ...
    NSDictionary* mRequestDict=[NSDictionary dictionaryWithObjectsAndKeys:mAppDelegate_.mModelForSplashScreen_.mEmployeeID_,@"UserID",mPartsArray,@"Data",[self getFilteredLabor:mLaborArray],@"Labor" , nil];
...
-(NSMutableArray*)getFilteredLabor:(NSMutableArray*)aLaborArray{
    NSMutableArray* lTempArray=[NSMutableArray new];
    for(int i=0;i<[mAppDelegate_.mPartLabourDataGetter_.mAddedPartsArray_ count];i++){
        NSPredicate* mPredicate=[NSPredicate predicateWithFormat:@"SELF.ASRLID = %@",[[mAppDelegate_.mPartLabourDataGetter_.mAddedPartsArray_ objectAtIndex:i] objectForKey:KID]];
        NSArray* mTempArray=[aLaborArray filteredArrayUsingPredicate:mPredicate];
        if([mTempArray count]>0){
        float hours=0.0;
        for(int j=0;j<[mTempArray count];j++){
            hours+=[[[mTempArray objectAtIndex:j] objectForKey:KHOURS] floatValue];
        }
      [lTempArray addObject:[NSDictionary dictionaryWithObjectsAndKeys:[[mAppDelegate_.mPartLabourDataGetter_.mAddedPartsArray_ objectAtIndex:i] objectForKey:KID],@"ASRLID",[NSString stringWithFormat:@"%f",hours],KHOURS, nil]];
    }
    }
        return lTempArray;
}
