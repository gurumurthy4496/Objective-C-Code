...
//        [mAppDelegate_ mModelForVehicleHistory_].mRODetailsArray_ = [mAppDelegate_.mModelForWalkAround_.mRepairOrderDetailsArray_ mutableCopy];
        ...
        [mAppDelegate_ mModelForVehicleHistory_].mRODetailsArray_ = [mAppDelegate_.mModelForWalkAround_.mRepairOrderDetailsArray_ mutableCopy];
        DLog(@"RO Details :-%@",[mAppDelegate_ mModelForVehicleHistory_].mRODetailsArray_);
...
- (UIView *)methodForReturnExpandedViewServicesArray:(NSArray *)aServicesArray :(UIView *)aExpandView {
    ...
    if (aServicesArray != nil) {
        ...
        __block int acceptLinesCount = 0;
        __block int declineLinesCount = 0;
        [aServicesArray enumerateObjectsUsingBlock:^(id object, NSUInteger index, BOOL *stop) {
            ...
            if ([[object objectForKey:@"Declined"] boolValue] == TRUE) {
                declineLinesCount++;
                DLog(@"declineLinesCount %d",declineLinesCount);
                [lServiceNameLabel setText:[NSString stringWithFormat:@"Declined Line %d: %@ - %@",declineLinesCount,[[object objectForKey:@"PayTypeCode"] objectForKey:@"Code"],[object objectForKey:@"ServiceName"]]];
                [lServiceNameLabel setTextColor:[UIColor redColor]];
                [lServiceNameLabel setHighlightedTextColor:[UIColor redColor]];
            }else {
                acceptLinesCount++;
                DLog(@"acceptLinesCount %d",acceptLinesCount);
                [lServiceNameLabel setText:[NSString stringWithFormat:@"Line %d: %@ - %@",acceptLinesCount,[[object objectForKey:@"PayTypeCode"] objectForKey:@"Code"],[object objectForKey:@"ServiceName"]]];
            ...
            //Price Label
            CGFloat lPriceLaborPartsTotal = [[object objectForKey:@"Price"] floatValue];//[[object  objectForKey:@"PriceLabor"] integerValue] + [[object objectForKey:@"PriceParts"] integerValue];
            ...
            if ([[object objectForKey:@"Declined"] boolValue] == TRUE) {
                if((mAppDelegate_.mMasterViewController_->mselectedSegment_==3)||([[[[SharedUtilities sharedUtilities] appDelegateInstance] isCustomHeaderViewFullOrLight] isEqualToString:@"FULL"])){

                CustomButtonForAddServicesWalkAround *button = [[CustomButtonForAddServicesWalkAround alloc] init];
                UIImage *image = [UIImage imageNamed:@"AddServicesWalkAround"];
                [button setBackgroundImage:image forState:UIControlStateNormal];
                [button setBackgroundImage:image forState:UIControlStateNormal];
                button.tag = index;
                button.mRepairOrderLinesDictionary_ = (NSMutableDictionary *)object;
                [button setFrame:CGRectMake(272, yPosition + (45 - image.size.height)/2, image.size.width, image.size.height)];
                [button addTarget:self action:@selector(addDeclinedServicesButtonAction:) forControlEvents:UIControlEventTouchUpInside];
              
                [button setHidden: ((mAppDelegate_.mMasterViewController_->mselectedSegment_==3)&&(mAppDelegate_.mModelForVehicleHistoryTableView_.mCurrentMode_==7))?TRUE:FALSE];
...
- (IBAction)addDeclinedServicesButtonAction:(CustomButtonForAddServicesWalkAround *)sender {
    ...
    if(mAppDelegate_.mMasterViewController_->mselectedSegment_==3){
        ...
        [[[SharedUtilities sharedUtilities]appDelegateInstance].mModelforOpenROSupportEngine_ RequestForAddServiceLine:mAppDelegate_.mModelForVehicleHistoryTableView_.mOpenROString_ SGCID:[lTempDict objectForKey:KSGCID]];
        }
    }
    else{
    ...
    DLog(@"%i",mAppDelegate_.mModelForVehicleHistoryTableView_.mCurrentMode_);
