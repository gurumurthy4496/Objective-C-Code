...
- (void)setFontsAndTextToSearchScreenView {
    if (!mAppDelegate_.mModelForEditCustomerScreen_.mIsBeginVehicle_) {
        ...
        [self.mSaveBtn_ setTitle:(mAppDelegate_.mSearchViewController_.mEditVehicleBtn_.selected || mAppDelegate_.mSearchVehicleInfoView_.mAddVehicleButton_.selected)?NSLocalizedString(@"Add_Button_Title", nil):NSLocalizedString(@"Save_Button_Title", nil) forState:UIControlStateNormal];
    }else {
        [self.mTitleLbl_ setText:NSLocalizedString(@"Edit_Vehicle_Title", nil)];
...
- (void)setLabelsAndTextFields {
    ...
    NSString *lTitleStr = @"VIN,Year,Make,Model,Mileage,License,Color";
                ...
                if ([pView isKindOfClass:[UITextField class]]) {
                    switch (((UITextField*)pView).tag) {
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            [mAppDelegate_.mModelForEditVehicleScreen_ setMColor_:((UITextField*)pView).text];
                            break;
                ...
                if ([pView isKindOfClass:[UITextField class]]) {
                    switch (((UITextField*)pView).tag) {
                        case 0:
                            ...
                            break;
                        case 1:
                            [((UITextField*)pView) setText:[NSString stringWithFormat:@"%@",mAppDelegate_.mModelForEditVehicleScreen_.mYear_]];
                            break;
                        case 2:
                        ...
                        case 5:
                            ...
                            break;
                        case 6:
                            [((UITextField*)pView) setText:mAppDelegate_.mModelForEditVehicleScreen_.mColor_];
                            break;
...
- (IBAction)saveBtnAction:(id)sender {
    ...
    if([self validationsForTextFields]) {
        ...
        if ([mAppDelegate_.mViewReference_ isKindOfClass:[SearchVehicleInfoView class]]) {
            ...
            if ([[mAppDelegate_.mSearchDataGetter_.mBeginCheckInSearchData_ objectForKey:@"Vehicles"] count] !=0) {
                ([[[mAppDelegate_.mSearchDataGetter_.mBeginCheckInSearchData_ objectForKey:@"Vehicles"] objectAtIndex:0] objectForKey:@"ID"]!=nil && ![[[[mAppDelegate_.mSearchDataGetter_.mBeginCheckInSearchData_ objectForKey:@"Vehicles"] objectAtIndex:0] objectForKey:@"ID"] isEqualToString:@"00000000000000000000000000000000"] )?[self requestToSaveVehicleDetails]:[self requestToAddVehicleDetails];
            }else {
                [self requestToAddVehicleDetails];
            }
        }
    }
}
...
- (void)textFieldDidBeginEditing:(UITextField *)textField{
    ...
    UIEdgeInsets contentInsets = UIEdgeInsetsMake(0.0, 0.0, [[[UIDevice currentDevice] systemVersion] floatValue] >= 8.0?(kKeyboardHeightForiOS8+_mToolBar_.frame.size.height):(kKeyboardHeight+_mToolBar_.frame.size.height), 0.0);
    self.mScrollView_.contentInset = contentInsets;
    self.mScrollView_.scrollIndicatorInsets = contentInsets;
...
#define MaxLength 17
#define MaxLengthForColor 20
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string { // return NO to not change text
    if (textField.tag == 0) {
        [mAppDelegate_.mModelForEditVehicleScreen_ setMVIN_:[textField.text stringByReplacingCharactersInRange:range withString:string]];
        DLog(@"length : %d", mAppDelegate_.mModelForEditVehicleScreen_.mVIN_.length);
        if (mAppDelegate_.mModelForEditVehicleScreen_.mVIN_.length == MaxLength) {
            [mAppDelegate_.mModelForSearchScreen_ threadRequestForVINDecoderForAddVehicle:mAppDelegate_.mModelForEditVehicleScreen_.mVIN_];
        }else if(mAppDelegate_.mModelForEditVehicleScreen_.mVIN_.length > MaxLength)
            return NO;
    }else  if (textField.tag == 6) {
        [mAppDelegate_.mModelForEditVehicleScreen_ setMColor_:[textField.text stringByReplacingCharactersInRange:range withString:string]];
        DLog(@"length : %d", mAppDelegate_.mModelForEditVehicleScreen_.mColor_.length);
        if (mAppDelegate_.mModelForEditVehicleScreen_.mColor_.length == MaxLengthForColor) {
        }else if(mAppDelegate_.mModelForEditVehicleScreen_.mColor_.length > MaxLengthForColor)
            return NO;
    }
    return YES;
}
