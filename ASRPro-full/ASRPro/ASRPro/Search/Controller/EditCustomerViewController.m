...
{
    if (self.view.superview.transform.tx == 100) {
...
- (void)setFontsAndTextToSearchScreenView {
    ...
    if ([mAppDelegate_.mViewReference_ isKindOfClass:[BeginVehicleCheckInViewViewController class]]) {
        [self.mEditCustomerTitleLbl_ setText:NSLocalizedString(@"Add_Customer_Title", nil)];
        [self.mSaveBtn_ setTitle:NSLocalizedString(@"Add_Button_Title", nil) forState:UIControlStateNormal];
    }else {
        ...
        [self.mSaveBtn_ setTitle:(mAppDelegate_.mSearchViewController_.mEditCustomerBtn_.selected || mAppDelegate_.mSearchCustomerInfoView_.mEditButton_.selected)?NSLocalizedString(@"Add_Button_Title", nil):NSLocalizedString(@"Save_Button_Title", nil) forState:UIControlStateNormal];
    }
    }else {
        [self.mEditCustomerTitleLbl_ setText:NSLocalizedString(@"Edit_Customer_Title", nil)];
        [self.mSaveBtn_ setTitle:(mAppDelegate_.mSearchViewController_.mEditCustomerBtn_.selected || mAppDelegate_.mSearchCustomerInfoView_.mEditButton_.selected)?NSLocalizedString(@"Add_Button_Title", nil):NSLocalizedString(@"Save_Button_Title", nil) forState:UIControlStateNormal];
    }
                    ...
                    switch (lTextField.tag) {
                        case 100: {
                            if (lTextField.text !=nil && ![lTextField.text isEqualToString:@""]) {
                                ...
                                [[SharedUtilities sharedUtilities] showAlertWithTitle:NSLocalizedString(@"lERROR", nil) message:NSLocalizedString(@"Enter_FirstName", nil)];
                                returnValue = FALSE;
                                return NO;
                            }
                        }
                        ...
                        case 101:{
                            if (lTextField.text !=nil && ![lTextField.text isEqualToString:@""]) {
                                ...
                                [[SharedUtilities sharedUtilities] showAlertWithTitle:NSLocalizedString(@"lERROR", nil) message:NSLocalizedString(@"Enter_LastName", nil)];
                                returnValue = FALSE;
                                return NO;
                            }
                        }
...
- (void)requestToAddCustomerDetails {
    ...
    if ([[NetworkMonitor instance] isNetworkAvailable]) {
        if ([mAppDelegate_.mViewReference_ isKindOfClass:[BeginVehicleCheckInViewViewController class]]|| [mAppDelegate_.mViewReference_ isKindOfClass:[EditCustomerViewController class]]) {
            ...
            [[SharedUtilities sharedUtilities] createLoadView];
        [mAppDelegate_.mRequestMethods_ postRequestToAddCustomerDetails:mAppDelegate_.mModelForEditCustomerScreen_.mCustomerID_ StoreID:mAppDelegate_.mModelForSplashScreen_.mStoreID_ CustomerNumber:@"" FirstName:mAppDelegate_.mModelForEditCustomerScreen_.mFirstName_ LastName:mAppDelegate_.mModelForEditCustomerScreen_.mLastName_ MiddleName:mAppDelegate_.mModelForEditCustomerScreen_.mMiddleName_ Email:mAppDelegate_.mModelForEditCustomerScreen_.mEmail_ HomePhone:mAppDelegate_.mModelForEditCustomerScreen_.mHomePhone_ WorkPhone:mAppDelegate_.mModelForEditCustomerScreen_.mWorkPhone_ MobilePhone:mAppDelegate_.mModelForEditCustomerScreen_.mMobilePhone_ Address1:mAppDelegate_.mModelForEditCustomerScreen_.mAddress1_ Address2:mAppDelegate_.mModelForEditCustomerScreen_.mAddress2_ Country:mAppDelegate_.mModelForEditCustomerScreen_.mCountry_ State:mAppDelegate_.mModelForEditCustomerScreen_.mState_ City:mAppDelegate_.mModelForEditCustomerScreen_.mCity_ Zipcode:mAppDelegate_.mModelForEditCustomerScreen_.mZipCode_ ContactEmail:mAppDelegate_.mModelForEditCustomerScreen_.mContactEmail_ ContactPhone:mAppDelegate_.mModelForEditCustomerScreen_.mPhone_ ContactSMS:mAppDelegate_.mModelForEditCustomerScreen_.mSMS_];
    } else {
        [[NetworkMonitor instance]displayNetworkMonitorAlert];
        return;
    }
...
- (IBAction)saveBtnAction:(id)sender {
    if([self validationsForTextFields]) {
        ...
        if ([mAppDelegate_.mViewReference_ isKindOfClass:[BeginVehicleCheckInViewViewController class]]) {
            ...
            [mAppDelegate_.mModelForEditCustomerScreen_.mCustomerID_ isEqualToString:@"00000000000000000000000000000000"]?[self requestToAddCustomerDetails]:[self requestToSaveCustomerDetails];
            }
        }
        }
        else {
...
//    NSLog(@"%@",NSStringFromCGAffineTransform(self.view.superview.transform));
    if( [[[UIDevice currentDevice] systemVersion] floatValue] < 8.0)
    {
