...
- (void)codeForCancelButton {
    ...
    [customButton setFrame:CGRectMake(self.frame.size.width - 120, 0, 60, 35)];
...
- (void)codeForSaveButton {
    UIButton *customButton = [UIButton buttonWithType:UIButtonTypeCustom];
    [customButton setFrame:CGRectMake(self.mClearButton_.frame.origin.x + self.mClearButton_.frame.size.width + 10, 0, 60, 35)];
...
- (void)codeToHideClearButton {
    DLog(@"%@",[[[SharedUtilities sharedUtilities] appDelegateInstance] mModelForCheckIn_].mCustomerAndVehicleInfoArray_);
    if ([[[[[SharedUtilities sharedUtilities] appDelegateInstance] mModelForCheckIn_].mCustomerAndVehicleInfoArray_ valueForKey:@"CustomerSignatureImageUrl"] length] == 0) {
        [self.mSaveButton_ setFrame:CGRectMake(self.mClearButton_.frame.origin.x + self.mClearButton_.frame.size.width + 10, 0, 60, 35)];

        [[self.mSaveButton_ titleLabel] setFont:[UIFont regularFontOfSize:15 fontKey:kFontNameHelveticaNeueKey]];
        [self.mSaveButton_ setTitle:@"Save" forState:UIControlStateNormal];
        [self.mSaveButton_ setTitle:@"Save" forState:UIControlStateSelected];
        [self.mSaveButton_ setUserInteractionEnabled:TRUE];
        [self.mCustomerSignatureUIImageView_ setUserInteractionEnabled:TRUE];
    }else {
        [self.mSaveButton_ setFrame:CGRectMake(self.mClearButton_.frame.origin.x + self.mClearButton_.frame.size.width - 20, 0, 80, 35)];

        [[self.mSaveButton_ titleLabel] setFont:[UIFont regularFontOfSize:15 fontKey:kFontNameHelveticaNeueKey]];
        [self.mSaveButton_ setTitle:@"Saved" forState:UIControlStateNormal];
        [self.mSaveButton_ setTitle:@"Saved" forState:UIControlStateSelected];
        [self.mClearButton_ setHidden:TRUE];
        [self.mSaveButton_ setUserInteractionEnabled:FALSE];
        [self.mCustomerSignatureUIImageView_ setUserInteractionEnabled:FALSE];
    }
}
